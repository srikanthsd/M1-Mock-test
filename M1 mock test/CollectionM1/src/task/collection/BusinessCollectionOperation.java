...
public class BusinessCollectionOperation {
	...
	public BusinessCollectionOperation() {
		...
		List<Review> reviewList1 = Arrays.asList(new Review("A", 4, "Good"), new Review("B", 2, "Not good"),
				new Review("C", 4, "Good"));
		List<Review> reviewList2 = Arrays.asList(new Review("A", 5, "Good"), new Review("B", 1, "Not good"),
				new Review("C", 3, "Good"), new Review("D", 4, "very Good"), new Review("E", 5, "nice"));

		List<Review> reviewList3 = Arrays.asList(new Review("A", 4, "Good"), new Review("B", 1, "Not good"));
	...
	/*
	 * method is used to sort the food items based on food type food has higher cost
...

	{
		...
		avgReview = avgReview.entrySet().stream()
				...
				.sorted((entry1, entry2) -> entry1.getKey().compareTo(entry2.getKey())).collect(HashMap::new,
						(map, entry) -> map.put(entry.getKey(), entry.getValue()), (m1, m2) -> m1.putAll(m2));
	...
	/*
	 * method shouldreturn a Map<Type,Map<String, List<Food>>> where Type is Food
	 * type enum for Veg and Non Veg String will hold the value of region which
	 * consist all food items based on region
	 */
